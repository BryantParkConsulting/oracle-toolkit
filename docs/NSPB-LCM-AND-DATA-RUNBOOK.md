...
### The duplicate-Class defect (found 2026-07-29) — the roster was loaded TWICE

Symetri's roster went into `Workforc` / `Scenario=Forecast` twice, and only the second
one was right:

| when | grid | Class |
| --- | --- | --- |
| 2026-07-22 | `symetri-workforce-roster-load.xlsx` (433 rows) | the employee's real business area — `CLASS_6/7/8/10/111/114/122` for 195 of them |
| 2026-07-28 | `symetri-workforce-roster-load-SUBMIT.xlsx` (464 rows) | all `Undefined_Class` |

The second load was the correction — CalcComp only reads `Undefined_Class` (§2 of
`WORKFORCE-LOAD-RECIPE.md`) — but **a reload does not remove the first blocks**. 195
employees ended up at both classes, so the roster form lists them twice and Total Class
doubles their salary. Verified live:

```
EMP_5471 / DEPT_27, SalRate:   Undefined_Class 97,129.20 | CLASS_6 97,129.20 | TC 194,258.40
control (should be, and is, empty):  CLASS_11 —   CLASS_119 —
```

Eric reported it as *"duplicate employees assigned to different classes"* and it read as a
broken NetSuite integration. **It is not** — the NetSuite saved search is not involved, and
nothing in Data Management needs changing. Confined to `Scenario=Forecast`; Budget and Actual
are clean.

**The fix**, driven off the first load's grid so the intersections are exact:

```
# Smart View route (no tenant object is created — an ad-hoc grid is just member names + Submit)
PYTHONUTF8=1 python clients/symetri/workforce/build-clear-grid.py
#   -> symetri-workforce-CLEAR-dupe-classes.xlsx, 195 rows x 5 props = 975 #Missing cells,
#      Undefined_Class never referenced, so a stray Submit cannot hurt the good data

# headless route, same thing via importdataslice
node tools/nspb-clear-symetri-dupe-classes.js --pilot        # EMP_5471, prints before/after
node tools/nspb-clear-symetri-dupe-classes.js --all --yes    # the 195

node tools/epm-run.js symetri runrule "AggAll" --yes         # recompute the Class rollups
```
