...
${nspbBundle?.state === 'active' ? `<div class="note"><b>Planning is already connected.</b> ${esc(nspbBundle.evidence)}.${nativeBudgets?.state === 'active' ? ` At the same time budgets are still being loaded natively (${esc(nativeBudgets.evidence)}). Confirming which one is intended to be authoritative is worth doing early, since it shapes what the model has to cover.` : ''}${conn?.integrations?.some(i => /pbcs|nspb/i.test(i.app)) ? ` Planning integrations go back to ${esc(conn.integrations.filter(i => /pbcs|nspb/i.test(i.app)).map(i => i.desde).sort()[0])}, so there is an established implementation to build on rather than a blank slate.` : ''}</div>` : ''}
<ul>
