...
(async () => {
  ...
  const dateFormat = flag('dateFormat', 'YYYYMMDD');

  // Proven Workforc roster POV, flat, Period first. Class (idx 6) and Department (idx 8)
  // are substituted per employee from the grid's row headers [emp, dept, class].
  // See docs/WORKFORCE-LOAD-RECIPE.md — properties MUST land at Class=Undefined_Class.
  const povTemplate = flag('pov',
    'BegBalance,No Year,Forecast,Base,USD,SUB_4,{class},No Location,{dept}').split(',');
  ...
  // Dates are loaded through the smart date columns; encode Excel dates as YYYYMMDD numbers.
  const DATE_COLS = new Set(['StartDate', 'EndDate', 'CalcStartDate', 'CalcEndDate']);

  let accepted = 0, rejected = 0, calls = 0;
  const rejSamples = [];
  for (const row of send) {
    const [emp, dept, cls] = row.headers;
    const pov = povTemplate.map(m => m === '{class}' ? cls : m === '{dept}' ? dept : m);
    for (let i = 0; i < accounts.length; i++) {
      ...
      const body = {
        ...
        ...(DATE_COLS.has(account) ? { dateFormat } : {}),
        dataGrid: { pov, columns: [[account]], rows: [{ headers: [emp], data: [val] }] },
      };
      ...
      calls++;
      if (r.ok && j) { accepted += j.numAcceptedCells || 0; rejected += j.numRejectedCells || 0; }
      else { rejected++; if (rejSamples.length < 5) rejSamples.push(`${emp}/${account}: ${r.status} ${text.slice(0, 100)}`); }
      if (calls % 200 === 0) console.log(`  … ${calls} cells sent (${accepted} ok)`);
    }
  }
  console.log(`\n  done: ${accepted} accepted, ${rejected} rejected, ${calls} calls`);
  if (rejSamples.length) console.log('  reject samples:\n   ' + rejSamples.join('\n   '));
